...
class Simulator(object):
  ...
  '''
  Run the simulator for self.frames number of frames.
  Images and progress is logged every self.log_freq frames.
  The simulator does not handle collisions right now.
  '''
  def Run(self):
    for f in range(self.frames):
      # log status
      if f%self.log_freq == 0:
...
class SimpleRandomSimulator(Simulator):
  def __init__(self, frames, log_freq, dt, max_objects, output_dir):
    super(SimpleRandomSimulator, self).__init__(frames, log_freq, dt,
                                                max_objects, output_dir)
...
if __name__ == '__main__':
  ...
  simulator = SimpleRandomSimulator(args.frames, args.log_freq, 0.1,
                                    args.max_objects, args.output_dir)
  simulator.Run()
